#pragma region private includes
...
pid speedPid[4], zAnglePid;

float coff = 0.4f; // 低通滤波系数
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM7)
	{
		...
		goal3 += zAngleTht + drive_velocity_x_goal + drive_velocity_y_goal;
		goal4 += zAngleTht - drive_velocity_x_goal + drive_velocity_y_goal;
		goal1 -= zAngleTht - drive_velocity_x_goal - drive_velocity_y_goal;
		goal2 -= zAngleTht + drive_velocity_x_goal - drive_velocity_y_goal;
